        ...
        public void Run(IEcsSystems systems)
        {
            ...
            foreach (var entity in playerFilter) 
            {
                ...
                foreach (var chase in chaserFilter)
                {
                    ...
                    var oldDist = Vector3.Distance(chaserEntity.Target.position, chaserMoveEntity.Rigidbody.transform.position);
                    var newDist = Vector3.Distance(playerEntity.PlayerTransform.position, chaserMoveEntity.Rigidbody.transform.position);
                    if (newDist < oldDist)
                    {
                        chaserEntity.Target = playerEntity.PlayerTransform;
                    }
